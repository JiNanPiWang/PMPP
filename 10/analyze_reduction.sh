...
analyze_file() {
    LOCAL_SRC_FILE=$1
    # 示例: simple_reduction.cu -> simple_reduction
    LOCAL_BASE_NAME=$(basename "$LOCAL_SRC_FILE" .cu)
    # 示例: simple_reduction_analysis_results
    ...
    # -f --set full: 运行全量指标分析。用于 Kernel 级的详细指标（如内存带宽、指令吞吐、发散率）。
    ...
    # --stats=true: 打印简单统计信息。用于系统级的整体执行时间、H2D/D2H 拷贝时间、Kernel 启动延迟。
...
# 1. 分析 simple_reduction.cu (线性发散的基线规约)
analyze_file "simple_reduction.cu"

# 2. 分析 improved_reduction.cu (最小化发散的顺序寻址规约)
